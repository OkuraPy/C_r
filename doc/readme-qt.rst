VibbeCoin-qt: Qt4 GUI for VibbeCoin
===============================
...
Alternatively, install Qt Creator and open the `VibbeCoin-qt.pro` file.

An executable named `VibbeCoin-qt` will be built.
...
A warning for people using the *static binary* version of VibbeCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of VibbeCoin is linked against libdb5.3.
...
installed causes VibbeCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
