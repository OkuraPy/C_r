...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("VibbeCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #VibbeCoinTEST2\r");
            Send(hSocket, "WHO #VibbeCoinTEST2\r");
        } else {
            // randomly join #VibbeCoin00-#VibbeCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #VibbeCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #VibbeCoin%02d\r", channel_number).c_str());
        }
